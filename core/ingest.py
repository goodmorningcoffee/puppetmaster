...
def extract_domain_from_filename(filepath: Path) -> str:
    ...
    # Use timestamp regex to split off the timestamp more precisely
    domain_part = re.sub(r'_\d{8}_\d{6}$', '', stem)
    # Convert remaining underscores to dots
    return domain_part.replace('_', '.')


def _clean_lines(f):
    """Yield lines from a file with NUL bytes stripped (streaming)."""
    for line in f:
...
def load_spiderfoot_csv(filepath: Path, show_progress: bool = True) -> Generator[Dict, None, None]:
    ...
    # Stream file and clean NUL bytes on the fly
    with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
        # Check for encoding replacement characters (U+FFFD) in first chunk
        chunk = f.read(8192)
        if '\ufffd' in chunk:
            print(f"  Warning: {filepath.name} contains replacement characters (encoding issues detected)")
        f.seek(0)
...
def load_all_exports(directory: Path, show_progress: bool = True) -> Dict:
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            print(f"  Warning: Skipping {csv_file.name} due to error: {e}")
            continue
